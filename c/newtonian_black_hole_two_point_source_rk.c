...
int main(void)
{
    ...
    const char *name = "newtonian_black_hole_two_point_source_rk.ppm";

    /*  Reset the radius of the black hole to be a point.                     */
    nbh_setup_reset_radius(0.01);
    nbh_rk_reset_time_increment(0.01);
    nbh_setup_reset_size(2048U, 2048U);

    /*  Use the template function to render the image.                        */
#ifdef _OPENMP
    nbh_prun(nbh_gravity2, nbh_stop2, nbh_checker_board, nbh_rk_path, name);
#else
    nbh_run(nbh_gravity2, nbh_stop2, nbh_checker_board, nbh_rk_path, name);
#endif
    return 0;
}
/*  End of main.                                                              */
