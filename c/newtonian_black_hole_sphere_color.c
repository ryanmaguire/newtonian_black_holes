...
int main(void)
{
    ...
    const char *name = "newtonian_black_hole_sphere_color.ppm";
    nbh_setup_reset_radius(2.0);

    /*  Use the template functions to render the image.                       */
#ifdef _OPENMP
    nbh_prun(nbh_gravity, nbh_stop, nbh_checker_board_sphere,
             nbh_euler_normalized_path, name);
#else
    nbh_run(nbh_gravity, nbh_stop, nbh_checker_board_sphere,
            nbh_euler_normalized_path, name);
#endif
    return 0;
}
